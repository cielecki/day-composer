...
export const UnifiedInputArea: React.FC<{
  ...
  const [waveformData, setWaveformData] = useState<number[]>(Array(WAVEFORM_HISTORY_LENGTH).fill(0));
  ...
  useEffect(() => {
    ...
    if (prevIsTranscribingRef.current && !isTranscribing && lastTranscription) {
      // Insert the transcription into the text input rather than sending it directly
      setMessage(prev => {
        const newMessage = prev.trim() ? `${prev} ${lastTranscription}` : lastTranscription;
  ...
  const updateWaveformData = () => {
    ...
    let volume = dataArray.reduce((acc, val) => Math.max(acc, val), 0) / 255;
  ...
  const handleSendMessage = () => {
    ...
    if (attachedImages.length > 0) {
      ...
      addUserMessage(finalMessage, newAbortController().signal, imageData);
    } else {
        ...
        <div className="attached-images-preview">
          {attachedImages.map(image => (
            <div className="attached-image" key={image.id}>
              ...
              <div className="attached-image-name">{image.name}</div>
              <button 
                className="attached-image-remove"
                onClick={() => handleRemoveImage(image.id)}
                aria-label={t('ui.input.removeFile').replace('{{filename}}', image.name)}
              >
      ...
      <div className="unified-input-area">
              ...
              <div className="waveform">
                {waveformData.map((level, index) => (
                  <div 
                    ...
                    style={{ 
                      height: `${level}%`,
                      opacity: Math.max(0.3, index / WAVEFORM_HISTORY_LENGTH)  // Fade out older samples
                    }}
                  />
                ))}
              </div>
        ...
        <div className="input-controls-bottom">
          ...
          <div className="input-controls-right">
            {/* Only show the cancel button when recording */}
            {isRecording && (
              <button 
            ...
            {/* Microphone button */}
            <button 
              className={`input-control-button mic-button ${isRecording ? 'recording' : ''} ${isTranscribing ? 'transcribing' : ''}`}
              onClick={handleMicrophoneClick}
              disabled={isTranscribing || isPlayingAudio}
              aria-label={isTranscribing ? t('ui.recording.transcribing') : isRecording ? t('ui.recording.stop') : t('ui.recording.start')}
            >
              {isTranscribing ? (
                <LucideIcon name="loader" className="spinner" size={20} />
              ) : isRecording ? (
            ...
            <button 
              ...
              disabled={(message.trim() === '' && attachedImages.length === 0) && !(isGeneratingResponse || isPlayingAudio)}
              aria-label={isGeneratingResponse || isPlayingAudio ? t('ui.recording.stop') : t('ui.input.send')}
            >
