...
export interface ObsidianTool<TInput> {
  ...
  getActionText: (input: TInput, output: string, hasResult: boolean, hasError: boolean) => string;
...
export class ObsidianTools {
  ...
  private tools: ObsidianTool<Record<string, string | boolean | number | undefined>>[] = [
    createDocumentTool,
    searchVaultTool,
    readDocumentTool,
   ...
   */
  getTools(): ObsidianTool<Record<string, string | boolean | number | undefined>>[] {
    return this.tools;
  }
   ...
   */
  getToolByName(name: string): ObsidianTool<Record<string, string | boolean | number | undefined>> | undefined {
   ...
   */
  async processToolCall(toolName: string, toolInput: Record<string, string | boolean | number | undefined>): Promise<{ result: string, isError: boolean }> {
    ...
    try {
      ...
      if (!tool) {
        throw new ToolExecutionError(t('errors.tools.unknown').replace('{{tool}}', toolName));
      }
      ...
      const errorMessage = error instanceof ToolExecutionError
        ...
        : t('errors.tools.execution')
            ...
            .replace('{{error}}', error.message || String(error) || 'Unknown error');
