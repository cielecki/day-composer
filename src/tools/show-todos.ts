...
const schema = {
  name: "show_todos",
  description: "Shows a selected few to-do items from a document or today's daily note with a visual display",
  input_schema: {
    type: "object",
    properties: {
      todos: {
        type: "array",
        description: "Specific todos to show (provide descriptions). If empty, will attempt to select a few relevant todos.",
        items: {
          type: "string"
        }
      },
      status: {
        type: "string",
        description: "Filter todos by status ('pending', 'completed', 'abandoned', 'moved', or 'all'). Default is 'pending'.",
      },
      max_items: {
        type: "number",
        description: "Maximum number of items to return. Default is 5. Set to 0 for unlimited."
      },
      file_path: {
        type: "string",
        description: "The specific file to search for to-dos (including .md extension). If not provided, searches only in today's daily note.",
      },
      interactive: {
        type: "boolean",
        description: "Whether to enable interactive checkboxes. Default is true."
      }
    },
    required: []
  }
...
export const showTodosTool: ObsidianTool<ShowTodosToolInput> = {
  ...
  getActionText: (input: ShowTodosToolInput, output: string, hasResult: boolean, hasError: boolean) => {
    const statusFilter = input?.status || 'pending';
    const maxItems = input?.max_items !== undefined ? input.max_items : 5;
    const hasSpecificTodos = Array.isArray(input?.todos) && input.todos.length > 0;
  ...
  execute: async (plugin: MyPlugin, params: ShowTodosToolInput): Promise<string> => {
    try {
      const { todos = [], status = 'pending', max_items = 5, file_path, interactive = true } = params;
      ...
      if (file_path) {
        ...
        if (!exists) {
          throw new ToolExecutionError(t('errors.files.notFound').replace('{{path}}', file_path));
        }
        ...
        if (!exists) {
          throw new ToolExecutionError(t('errors.files.todayNotExists'));
        }
      ...
      if (todos.length > 0) {
        ...
        matchedTasks = statusFiltered.filter(task => 
          todos.some(todoText => 
            task.description.toLowerCase().includes(todoText.toLowerCase())
          )
        );
      } else {
      ...
      const limitedTasks = max_items > 0 ? matchedTasks.slice(0, max_items) : matchedTasks;
      ...
      if (error instanceof ToolExecutionError) {
        ...
        throw new ToolExecutionError(`Error showing to-do items: ${error.message || 'Unknown error'}`);
      }
    }
  },
  
  // Custom renderer for the tool result
  renderResult: (result: string, input: ShowTodosToolInput): React.ReactNode => {
    try {
      ...
      const handleCheckTodo = async (description: string) => {
        try {
          ...
          // @ts-ignore
          new window.Notice(t('errors.todos.check').replace('{{error}}', e.message || 'Unknown error'));
        }
      };
      ...
      const handleUncheckTodo = async (description: string) => {
        try {
          ...
          // Show error notification
          // @ts-ignore
          new window.Notice(t('errors.todos.uncheck').replace('{{error}}', e.message || 'Unknown error'));
        }
      };
      ...
      const handleAbandonTodo = async (description: string) => {
        try {
          ...
          // Show error notification
          // @ts-ignore
          new window.Notice(t('errors.todos.abandon').replace('{{error}}', e.message || 'Unknown error'));
        }
      };
      ...
      return React.createElement('div', { className: 'todo-render-error' },
