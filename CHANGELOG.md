...
- **Vault Exploration Tools**: Added two new tools for comprehensive vault browsing and navigation:
  - **List Directory Tool**: New `list_directory` tool that provides structured directory and file listing for any vault path. Features include optional recursive listing to show complete directory trees, file type filtering (markdown, images, PDFs, etc.), toggleable inclusion of files and folders, file size information, and visual formatting with folder/file icons. Perfect for AI to understand vault structure and help users navigate their content organization.
  - **Find Files by Tag Tool**: New `find_files_by_tag` tool that searches for files containing specific tags across the entire vault. Supports both in-content tags (#tag) and frontmatter tags, with options for exact matching vs. nested tag matching (e.g., 'project' matches 'project/work'). Includes line number reporting for content tags, optional file content snippets, heading extraction, and configurable result limits. Enables AI to quickly locate tagged content and understand the user's tagging system.
